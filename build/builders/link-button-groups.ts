 ...
 */
function buildPrimaryImgProps(iconProps: Record<string, unknown>): Record<string, unknown> {
    const srcMask = (iconProps.dataImgFeature === 'colored' && iconProps.dataSrcMask)
 ...
 */
export function buildLinkButtonGroupHTML(pageName: string, groupId: string): string {
    const jsonPath = resolve(__dirname, '..', 'configs', 'link-button-groups', `${pageName}.json`);
    ...
    if (!group || !Array.isArray(group.buttons) || group.buttons.length === 0) return '';

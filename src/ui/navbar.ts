...
import { extractPageName, normalizeInternalPath } from "../core/utils.js";
  ...
  document.querySelectorAll(".dropdown").forEach((dropdown) => {
    ...
    dropdown.addEventListener("hide.bs.dropdown", (event: Event) => {
      ...
      menu.addEventListener(
        "transitionend",
        function handler(evt: TransitionEvent) {
...
export function initMobileNavbarBrandScroll(): void {
  ...
  const brandText = pageTarget
    ? pageTarget.querySelector(".navbar-brand-text")
    : null;
...
export function updateNavbarBrandText(): void {
  const brandText = document.querySelector(
    "#navbar-brand-page-slide .navbar-brand-text",
  );
  if (!brandText) return;

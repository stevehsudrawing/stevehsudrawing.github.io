...
import { initImageLoadingOpacity, applyColoredImage } from "../ui/img-utils.js";
import { initSvgInjection } from "../ui/svg-utils.js";
import { errMsg, isInternalPage, setElementAttributes } from "../core/utils.js";
...
export async function showQRCodeModal(
  ...
  (modalElement as unknown as Record<string, unknown>)._qrIconProps =
    imgProperties || null;
  ...
  const colorDark =
    computedStyles.getPropertyValue("--bs-body-color").trim() || "#000000";
  const pageBg =
    computedStyles.getPropertyValue("--bs-body-bg").trim() || "#ffffff";
  ...
  const mergedProperties: Record<string, unknown> = Object.assign(
    {},
  ...
  if (titleEl) {
    ...
    const altText = i18nAltKey
      ? translate(i18nAltKey)
      : (mergedProperties.alt as string | undefined);
  ...
  if (openLinkBtn) {
    const isInternal =
      typeof isInternalPage === "function" && isInternalPage(linkUrl);

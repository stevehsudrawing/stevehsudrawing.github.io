...
import { applyColoredImage, markImageLoaded } from "../ui/img-utils.js";
 ...
 */
export function shouldConfirmExternalLink(
  ...
  if (typeof isInternalPage === "function" && isInternalPage(href))
    return false;
...
export function showExternalLinkConfirmation(
  ...
  (modalElement as unknown as Record<string, unknown>)._confirmIconProps =
    imgProperties || null;
...
export function handleExternalLinkShowQR(): void {
  ...
  const url = (modalElement as unknown as Record<string, unknown>)
    ._confirmUrl as string | undefined;
  const iconProps = (modalElement as unknown as Record<string, unknown>)
    ._confirmIconProps as Record<string, unknown> | null;

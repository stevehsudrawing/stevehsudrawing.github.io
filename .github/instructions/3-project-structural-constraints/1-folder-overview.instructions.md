...
| `src/composables/`                  | **Vue composables** - reactive state + side-effects extracted from core/ui modules. One concern per file (`useXxx.ts`). Depends on `types/`, `core/`.              | New composable when extracting reactive logic                        |                                        |
| `src/plugins/`                      | **Vue plugins** - global provide/inject registrations (e.g. `i18n.ts`). Loaded via `app.use()` in `main.ts`.                                                       | New plugin when adding app-level injection                           |                                        |
| `src/components/`                   | **Vue SFCs** - PascalCase filenames, organized by function: `layout/` (navbar, footer), `ui/` (buttons, indicators), `modals/` (settings, confirmations).          | New `.vue` component in the appropriate sub-directory                |                                        |
...
| `src/main.ts`                       | Vite entry point - full-feature pages (all pages)                                                                                                                  | -                                                                    |                                        |

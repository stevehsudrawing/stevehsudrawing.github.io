...
| `src/*.html`       | MPA entry points (index, about, artworks-and-videos, gallery, blogs-and-sponsor, chatting, softwares, copyright-notice). Static `<noscript>` + `<div id="app">`.                                                                                                                                                         | —                                                     |
...
| `src/components/`  | **Vue SFCs** — organized by function: `nav/` (navigation bars, page chain), `ui/` (shared primitives), `render-functions/` (template-less render-function components), `images/` (image components), `modals/` (dialog overlays), `cards/` (content cards), `buttons/` (clickable elements), `links/` (link components). | New `.vue` component in the appropriate sub-directory |
...
| `src/configs/`     | **Runtime configs** — TS constants (`site-meta.ts`, `language-list.ts`, `pages.ts`, `theme-options.ts`, `i18n/`) + JSON data (`link-cards/`, `link-button-groups/`, `picture-list/`).                                                                                                                                    | New config file for runtime data                      |
...
- Put Vue SFCs in `src/components/` by function:
  ...
  - `render-functions/` — Template-less components building VNodes via
    render functions (`TooltipTrigger`, `HastFragment`)
  - `images/` — Image components (`FeatureAwarePicture`, `ColoredImg`, `TypeAwareImage`)

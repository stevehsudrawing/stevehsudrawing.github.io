...
- A centre overlay icon is rendered from `imgProperties` (hast-format properties; see [§4.5.1.2](5-link-cards.md#4512-property-naming-hast-convention)). Custom icons replace the default link icon.
- **Modal footer** (left to right): [Open Link] [Share] [Download] (me-auto) [Close].
    - **Open Link** (`bi-box-arrow-up-right`, icon-only): Switches to the external link confirmation modal ([§4.17](17-external-link-confirmation.md#417-external-link-confirmation)) via `hidden.bs.modal` transition. Hidden for internal links (`isInternalPage`). Has `aria-label` + tooltip (i18n key `text-open`).
